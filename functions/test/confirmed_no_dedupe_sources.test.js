...
test("取込ロジック(import_*)はまだcallableへ接続していない(index.jsがconfirmed/から読み込むのは、認可つきで公開するAPI(access_role・import_api・winner_mail_api・winner_send_api)だけ)", () => {
  ...
  assert.deepEqual(required, ["access_role", "import_api", "winner_mail_api", "winner_send_api"]);
