...
test("取込ロジック(import_*)はまだcallableへ接続していない(index.jsがconfirmed/から読み込むのは、認可つきで公開するAPI(access_role・import_api・winner_mail_api・winner_send_api・pass_api・reminder_api・event_create_api(Phase 11A: 新方式イベントの作成。admin専用))だけ)", () => {
  ...
  assert.deepEqual(required, ["access_role", "import_api", "event_create_api", "winner_mail_api", "winner_send_api", "pass_api", "reminder_api"]);
